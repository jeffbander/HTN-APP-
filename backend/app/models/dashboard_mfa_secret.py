"""
DashboardMfaSecret model for storing TOTP secrets for dashboard users.
"""
import json
import secrets
import string
...
class DashboardMfaSecret(db.Model):
    """Stores TOTP secrets (encrypted) and backup codes for dashboard MFA."""
    __tablename__ = 'dashboard_mfa_secrets'

    id = db.Column(db.Integer, primary_key=True)
    dashboard_user_id = db.Column(
        db.Integer, db.ForeignKey('dashboard_users.id'), nullable=False, unique=True
    )
    ...
    dashboard_user = db.relationship(
        'DashboardUser', backref=db.backref('mfa_secret', uselist=False)
    )
    ...
    def __repr__(self):
        return f'<DashboardMfaSecret user={self.dashboard_user_id} type={self.mfa_type}>'
